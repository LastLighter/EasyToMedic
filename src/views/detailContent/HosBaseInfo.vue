...
<template>
    <div class="container" v-if="hos != undefined">
        ...
        <div class="bottom-entry">
            <div class="address line-spacing inner-rule">具体地址: {{ hos.hospital.param.fullAddress }}</div>
            <div class="address line-spacing inner-rule">推荐路线: {{ hos.hospital.route }}</div>
            <div class="hospital-rule">
                <div class="title line-spacing">医院介绍</div>
                <div class="line-spacing inner-rule">{{ hos.hospital.intro }}</div>
            </div>
        </div>
    </div>
</template>
